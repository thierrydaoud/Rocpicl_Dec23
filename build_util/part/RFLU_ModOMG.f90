...
! ******************************************************************************
    ...
    CALL RegisterFunction(global,'RFLU_CheckGridOMG', &
                          "../../utilities/part/RFLU_ModOMG.F90")
...
! ******************************************************************************
    ...
    IF ( pGrid%nTetsTot > 0 ) THEN 
      ...
      IF ( global%error /= ERR_NONE ) THEN
        ...
        END IF ! global%verbLevel           
        CALL ErrorStop(global,global%error,151)
      END IF ! global%error
    END IF ! pGrid
...
! *****************************************************************************
    ...
    DO iPatch = 1,pGrid%nPatches
      ...
      IF ( global%error /= ERR_NONE ) THEN 
        CALL ErrorStop(global,ERR_ALLOCATE,350,'pPatch%bf2c')
      END IF ! global%error

      ALLOCATE(pPatch%bf2v(4,pPatch%nBFacesMax),STAT=errorFlag)
      global%error = errorFlag
      IF ( global%error /= ERR_NONE ) THEN 
        CALL ErrorStop(global,ERR_ALLOCATE,356,'pPatch%bf2v')
      END IF ! global%error  
...
! ******************************************************************************
    ...
    global%error = errorFlag   
    IF ( global%error /= ERR_NONE ) THEN 
      CALL ErrorStop(global,ERR_FILE_OPEN,521,iFileName)
    END IF
...
! ******************************************************************************
    ...
    IF ( (nHex > 0) .OR. (nWedge > 0) .OR. (nOct > 0) .OR. (nDomain > 1) ) THEN 
      CALL ErrorStop(global,ERR_REACHED_DEFAULT,537)
    END IF ! nHex
...
! ******************************************************************************
    ...
    global%error = errorFlag
    IF ( global%error /= ERR_NONE ) THEN 
      CALL ErrorStop(global,ERR_ALLOCATE,549,'grid%xyz')
    END IF ! global%error
...
! ******************************************************************************
    ...
    IF ( pGrid%nTetsMax > 0 ) THEN 
      ...
      global%error = errorFlag 
      IF ( global%error /= ERR_NONE ) THEN 
        CALL ErrorStop(global,ERR_ALLOCATE,570,'pGrid%tet2v')
      END IF ! global%error
    ELSE 
      NULLIFY(pGrid%tet2v)
...
! ******************************************************************************
    ...
    IF ( nCellsInDomain /= pGrid%nCells ) THEN 
      CALL ErrorStop(global,ERR_REACHED_DEFAULT,614)
...
! ******************************************************************************
    ...
    global%error = errorFlag
    IF ( global%error /= ERR_NONE ) THEN 
      CALL ErrorStop(global,ERR_ALLOCATE,632,'pRegion%patches')
    ...
    DO iPatch = 1,pGrid%nPatches 
      ...
      global%error = errorFlag
      IF ( global%error /= ERR_NONE ) THEN 
        CALL ErrorStop(global,ERR_ALLOCATE,655,'pPatch%bTri2v')
      END IF ! global%errorFlag

      ALLOCATE(dummyStringArray(pPatch%nBTris),STAT=errorFlag)
      global%error = errorFlag
      IF ( global%error /= ERR_NONE ) THEN 
        CALL ErrorStop(global,ERR_ALLOCATE,661,'dummyStringArray')
      END IF ! global%errorFlag
      ...
      global%error = errorFlag
      IF ( global%error /= ERR_NONE ) THEN 
        CALL ErrorStop(global,ERR_DEALLOCATE,676,'dummyStringArray')
      END IF ! global%errorFlag
...
! ******************************************************************************
    ...
    global%error = errorFlag   
    IF ( global%error /= ERR_NONE ) THEN 
      CALL ErrorStop(global,ERR_FILE_CLOSE,704,iFileName)
    END IF ! global%error  
